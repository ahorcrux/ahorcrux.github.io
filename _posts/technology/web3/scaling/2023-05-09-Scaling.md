...
+ **DA**
  ...
  *数据可用性*，区块生产者（矿工/验证者）必须公布并提供他们生产区块的交易数据，以便全节点来检查他们的工作。如果节点跑路，如何保证layer2 state的持久化，防止Layer2数据丢失导致资产锁死？
  ...
  ![starkware存储方案](https://github.com/xavier-warmhole/open-doc-img/blob/main/tech/web3/scaling/s_starkware%E5%AD%98%E5%82%A8%E6%96%B9%E6%A1%88.png)
...
+ **状态通道（State Channel）**
  ...
  ![s_state_channel](https://github.com/xavier-warmhole/open-doc-img/blob/main/tech/web3/scaling/s_%E7%8A%B6%E6%80%81%E9%80%9A%E9%81%93%E6%B5%81%E7%A8%8B%E5%9B%BE.png)
...
+ **侧链（Side Chain）**通过双向跨链桥（Two-way-pegged Cross-chain bridge）与主网交互， 有自己的共识、安全性。
  deposit时，主链质押token,侧链生成wrapped token ，即主链lock 侧链mint。
  withdraw时，侧链burn，主链unlock。
  
  ![s_侧链](https://github.com/xavier-warmhole/open-doc-img/blob/main/tech/web3/scaling/s_%E4%BE%A7%E9%93%BE%E7%BB%93%E6%9E%84.png)
...
+ **Plasma Chain**主-子链结构，子链是一个锚定到以太坊主网的独立区块链，但却在链下执行交易，有自己的区块验证机制。 Plasma 链有时被称作“子”链，其本质是以太坊主网的较小副本。 Plasma 链使用欺诈证明（如乐观卷叠）来仲裁争议。
  
  ![a](https://github.com/xavier-warmhole/open-doc-img/blob/main/tech/web3/scaling/s_plasma_%E7%BB%93%E6%9E%84%E5%9B%BE.png)
...
+ **Rollup**
  ...
  1. ***OP Rollup***Op-rollup是一种扩容以太坊的方法，涉及将计算和状态存储移至链下。 乐观卷叠在以太坊之外执行交易，但将交易数据作为 calldata 发布到主网。Op-rollup运营商validator将多个链下交易大批量捆绑在一起，然后再提交到以太坊。 这种方法可以将固定成本分散到每批中的多笔交易中，从而降低最终用户的费用。 乐观卷叠还使用压缩技术来减少发布在以太坊上的数据量。
     
     ![s_op_rollup](https://github.com/xavier-warmhole/open-doc-img/blob/main/tech/web3/scaling/s_op_rollup_arb.png)
  ...
  2. ***ZK Rollup***
     ...
     ![s_zk_rollup](https://github.com/xavier-warmhole/open-doc-img/blob/main/tech/web3/scaling/s_zk_rollup_%E6%B5%81%E7%A8%8B%E5%9B%BE.png)
...
+ **Validium**
  ...
  当前最新的技术方案，综合以上方案的优点。
  数据不上传到Layer1，由多个Validator托管（其实属于Sidechain类别，但由于是Rollup衍生出来的方案，所以大家更愿意与Rollup一起讨论比较）
...
![starkex](https://github.com/xavier-warmhole/open-doc-img/blob/main/tech/web3/scaling/s_layer3_starkex.png)
